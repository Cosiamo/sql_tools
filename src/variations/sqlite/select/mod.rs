...
use crate::{clauses::select::OrderBy, data_types::SQLDataTypes, Error, SQLVariation};
...
pub(crate) fn build_select_sqlite(mut select_props: SelectProps) -> Result<Vec<Vec<Box<SQLDataTypes>>>, Error> {
    ...
    let count_query = stmt.query_map([], |row| {
        Ok(row.get(0)?)
    })?;
    for res in count_query {
        let row = res?;
        count = if let Some(val) = row { Some(val) } else { Some(0usize) }
    }
    ...
    for n in 0..nthreads {
        ...
        let sql = format!("SELECT * FROM ({}) WHERE rn >= {} and rn <= {}", query, start, end);
        // println!("{:?}", stmt);
