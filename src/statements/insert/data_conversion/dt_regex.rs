...
pub static DATE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"^[0-3]?[0-9].[0-3]?[0-9].(?:[0-9]{2})?[0-9]{2}$").unwrap());
// is_datetime
...
pub static DT24: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"^[0-3]?[0-9].[0-3]?[0-9].(?:[0-9]{2})?[0-9]{2} ([01]?[0-9]|2[0-3]):[0-5][0-9]$")
        .unwrap()
});
...
// is_time_24h
pub static H24: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"^([01]?[0-9]|2[0-3]):[0-5][0-9]$").unwrap());
pub static H24_W_S: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"^([01]?[0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9]$").unwrap());
// is_time_12h
pub static H12: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"^([1-9]|0[1-9]|1[0-2]):[0-5][0-9] ([AaPp][Mm])$").unwrap());
pub static H12_W_S: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"^([1-9]|0[1-9]|1[0-2]):[0-5][0-9]:[0-5][0-9] ([AaPp][Mm])$").unwrap()
});
// is_number
pub static NUM: Lazy<Regex> =
